# FormatPrice

`namespace` [TigerTrade.Tc](../../../../TigerTrade.Tc.md).[Data](../../../Data.md)\
`class` [Symbol](../../Symbol.cs.md)

Описание

### Синтаксис

```csharp
public string FormatPrice(long price, bool f = false)
public string FormatPrice(long price, IFormatProvider provider, bool f = false)
public string FormatPrice(double price, bool f = false)
public string FormatPrice(double? price, bool f = false)
```

<mark style="color:yellow;">`price`</mark> _<mark style="color:red;">`long`</mark>_\
_Описание_

<mark style="color:yellow;">`provider`</mark> _<mark style="color:red;">`IFormatProvider`</mark>_\
_Описание_

<mark style="color:yellow;">`f`</mark> _<mark style="color:red;">`bool`</mark>_\
_Описание_

<mark style="color:yellow;">`price`</mark> _<mark style="color:red;">`double`</mark>_\
_Описание_

### Пример

```csharp
// example
```
